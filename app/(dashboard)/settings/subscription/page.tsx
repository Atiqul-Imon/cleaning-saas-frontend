...
export default function SubscriptionPage() {
  ...
  const handleUpdatePlan = async (planType: 'FREE' | 'SOLO' | 'SMALL_TEAM') => {
    if (planType === 'FREE') {
      ...
      setUpdating(true);
      try {
        const { url } = await apiClient.post<{ sessionId: string; url: string }>('/payments/create-checkout-session', { planType });
        if (url) {
  ...
  const usagePercentage = usageStats ? (usageStats.currentMonthUsage / usageStats.monthlyLimit) * 100 : 0;
  const isOverLimit = usageStats && usageStats.currentMonthUsage >= usageStats.monthlyLimit;
  const isNearLimit = usageStats && usageStats.currentMonthUsage >= usageStats.monthlyLimit * 0.8 && !isOverLimit;
  ...
  return (
    <Section background="subtle" padding="lg">
      <Container size="lg">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-extrabold text-[var(--gray-900)] mb-2">Subscription Management</h1>
        ...
        {subscription && usageStats && (
          <Card variant="elevated" padding="lg" className="mb-8">
            ...
            <Card variant="outlined" padding="md" className="bg-[var(--gray-50)] mb-4">
              <Stack spacing="sm">
                <Stack direction="row" justify="between" align="center">
                  <span className="text-sm font-semibold text-[var(--gray-700)]">This Month's Usage</span>
            ...
            <p className="text-sm text-[var(--gray-600)]">
              Current period ends: {new Date(subscription.currentPeriodEnd).toLocaleDateString('en-GB')}
            </p>
          </Card>
        )}
                    ...
                    <ul className="space-y-2">
                      {plan.features.map((feature, idx) => (
                        <li key={idx} className="flex items-start text-sm text-[var(--gray-700)]">
                          <svg className="w-5 h-5 text-[var(--success-600)] mr-2 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
