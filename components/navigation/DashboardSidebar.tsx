...
interface DashboardSidebarProps {
  userRole?: { role: string; id: string } | null;
}

export default function DashboardSidebar({ userRole: initialUserRole }: DashboardSidebarProps) {
  const { userRole } = useUserRole();
  const pathname = usePathname();
  const role = userRole || initialUserRole;

  const isOwner = role?.role === 'OWNER';
  const isCleaner = role?.role === 'CLEANER';
  ...
  const navItems = isOwner ? ownerNavItems : isCleaner ? cleanerNavItems : [];
